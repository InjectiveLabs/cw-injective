...
use cw_multi_test::{AddressGenerator, App, StargateAccepting, StargateFailing};

pub enum InjectiveStargate {
    Accepting(StargateAccepting),
    Failing(StargateFailing),
}
use cw_multi_test::{AppResponse, BankKeeper, BasicAppBuilder, CosmosRouter, Module, Router, WasmKeeper};
...
fn no_init<BankT, CustomT, WasmT, StakingT, DistrT, IbcT, GovT>(
    _: &mut Router<BankT, CustomT, WasmT, StakingT, DistrT, IbcT, GovT, InjectiveStargate>,
    _: &dyn Api,
    _: &mut dyn Storage,
) {
}
...
fn copy_binary(binary: &Binary) -> Binary {
    ...
    c.clone_from_slice(&binary.to_vec());
    Binary::new(c)
}
