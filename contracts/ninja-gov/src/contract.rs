...
pub fn receive_cw20(
    ...
    if config.ninja_token != deps.api.addr_canonicalize(info.sender.as_str())? {
...
pub fn create_poll(
    ...
    let poll_execute_data = if let Some(poll_execute_msg) = poll_execute_msg {
        ...
        if target_contract.eq(&config.admin_manager) || target_contract.eq(&contract_raw) {
            return Err(StdError::generic_err(
                "Can not make a normal pool targeting the admin_manager or ninja-gov contract",
            ));
        }
        Some(ExecuteData {
...
pub fn end_poll(deps: DepsMut, env: Env, poll_id: u64) -> StdResult<Response> {
    ...
    let (quorum, staked_weight) = if state.total_share.u128() == 0 {
        ...
        let staked_weight = load_token_balance(
            &deps.querier,
            deps.api.addr_humanize(&config.ninja_token)?.to_string(),
            &state.contract_addr,
        )?
    ...
    if tallied_weight == 0 || quorum < target_quorum {
        ...
        if !a_poll.deposit_amount.is_zero() {
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps.api.addr_humanize(&config.ninja_token)?.to_string(),
                funds: vec![],
...
fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    Ok(ConfigResponse {
        ...
        ninja_token: deps.api.addr_humanize(&config.ninja_token)?.to_string(),
