...
fn proper_initialization() {
    ...
    let res: InitResponse<InjectiveMsg> = init(&mut deps, env, msg).unwrap();
...
fn buy_limit() {
    ...
    let _res: InitResponse<InjectiveMsg> = init(&mut deps, env, msg).unwrap();
    ...
    if let CosmosMsg::Custom(InjectiveMsg::Swap(ExchangeMsg::Trade {
        trader_addr,
        offer_coin,
        ask_denom,
    })) = &res.messages[0]
