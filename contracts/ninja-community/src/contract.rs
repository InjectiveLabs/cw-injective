...
/// `amount` of NINJA token to `recipient` for community purpose
pub fn spend(
    deps: DepsMut,
    info: MessageInfo,
    recipient: String,
    ...
    let ninja_token = deps.api.addr_humanize(&config.ninja_token)?.to_string();
    Ok(Response::new()
        .add_message(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: ninja_token,
            funds: vec![],
