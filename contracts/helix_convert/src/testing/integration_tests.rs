...
use injective_test_tube::{Account, Bank, Exchange, Gov, InjectiveTestApp, Module, Runner, RunnerError, RunnerResult, SigningAccount, Wasm};
use injective_test_tube::RunnerError::{ExecuteError, GenericError, QueryError};
...
use crate::testing::test_utils::{create_limit_order, launch_spot_market, OrderSide, store_code, init_contract_and_get_address, must_init_account_with_funds, query_all_bank_balances, query_bank_balance, set_route_and_assert_success, pause_spot_market, fund_account_with_some_inj, init_contract_with_fee_recipient_and_get_address};
...
fn happy_path_two_hops_single_price_level_swap() {
    ...
    let query_result: FPDecimal = wasm
        ...
        .unwrap();
    assert_eq!(query_result, expected_atom_estimate_quantity, "incorrect swap result estimate returned by query");
    ...
    let to_balance = query_bank_balance(&bank, ATOM, swapper.address().as_str());
    assert_eq!(from_balance, FPDecimal::zero(), "some of the original amount wasn't swapped");
    assert_eq!(to_balance, expected_atom_estimate_quantity.int(), "swapper did not receive expected amount");
...
fn happy_path_three_hops_quote_conversion_swap() {
    ...
    let spot_market_1_id =
        launch_spot_market(&exchange, &owner, ETH, USDT);
    let spot_market_2_id =
        launch_spot_market(&exchange, &owner, ATOM, USDC);
    let spot_market_3_id =
        launch_spot_market(&exchange, &owner, USDC, USDT);

    let contr_addr = init_contract_and_get_address(&wasm, &owner, &[coin(10_000_000_000, USDC), coin(10_000_000_000, USDT)]);
    set_route_and_assert_success(&wasm, &owner, &contr_addr, ETH, ATOM, vec![spot_market_1_id.as_str().into(), spot_market_3_id.as_str().into(), spot_market_2_id.as_str().into()]);
    ...
    assert_eq!(query_result, FPDecimal::must_from_str("2889.64"), "incorrect swap result estimate returned by query");
...
fn happy_path_simple_buy_swap() {
    ...
    assert_eq!(contract_balances_after.len(), 1, "wrong number of denoms in contract balances");
    assert_eq!(FPDecimal::must_from_str(contract_balances_after[0].amount.as_str()), expected_contract_balances_after, "contract balance changed unexpectedly after swap");
}
...
fn happy_path_external_fee_receiver() {
    ...
    let contr_addr = init_contract_with_fee_recipient_and_get_address(&wasm, &owner, &[coin(10_000_000_000, USDT)], &fee_recipient);
    set_route_and_assert_success(&wasm, &owner, &contr_addr, ETH, ATOM, vec![spot_market_1_id.as_str().into(), spot_market_2_id.as_str().into()]);
    ...
    let buy_orders_nominal_total_value = FPDecimal::from(201000u128) * FPDecimal::from(5u128) + FPDecimal::from(195000u128) * FPDecimal::from(4u128) + FPDecimal::from(192000u128) * FPDecimal::from(3u128);
    let relayer_sell_fee = buy_orders_nominal_total_value * FPDecimal::must_from_str(&format!("{}", DEFAULT_TAKER_FEE * DEFAULT_ATOMIC_MULTIPLIER * DEFAULT_RELAYER_SHARE));
    ...
    let sell_orders_nominal_total_value = FPDecimal::from(800u128) * FPDecimal::from(800u128) + FPDecimal::from(810u128) * FPDecimal::from(800u128) + FPDecimal::from(820u128) * FPDecimal::from(800u128) + FPDecimal::from(830u128) * expected_nominal_buy_most_expensive_match_quantity;
    let relayer_buy_fee = sell_orders_nominal_total_value * FPDecimal::must_from_str(&format!("{}", DEFAULT_TAKER_FEE * DEFAULT_ATOMIC_MULTIPLIER * DEFAULT_RELAYER_SHARE));
    ...
    let query_result: FPDecimal = wasm
        ...
        .unwrap();
    assert_eq!(query_result, FPDecimal::must_from_str("2888.222"), "incorrect swap result estimate returned by query");
    ...
    assert_eq!(fee_recipient_balance[0].denom, USDT, "fee recipient did not receive fee in expected denom");
    assert_eq!(FPDecimal::must_from_str(fee_recipient_balance[0].amount.as_str()), expected_fee_for_fee_recipient.int(), "fee recipient did not receive expected fee");
}
...
fn multiple_fund_denmos_passed() {
    ...
    let execute_result = wasm.execute(
        ...
        &vec![coin(10, ATOM), coin(12, ETH)],
        &swapper,
    );
    assert!(execute_result.unwrap_err().to_string().contains("Only 1 denom can be passed in funds"), "wrong error message");
...
fn zero_minimum_amount_to_receive() {
    ...
    assert!(query_result.unwrap_err().to_string().contains("Generic error: from_quantity must be positive: query wasm contract failed"), "incorrect error returned by query");
...
fn negative_minimum_amount_to_receive() {
    ...
    let from_balance = query_bank_balance(&bank, ETH, swapper.address().as_str());
    let to_balance = query_bank_balance(&bank, ATOM, swapper.address().as_str());
    assert_eq!(from_balance, FPDecimal::from(12u128), "wrong from balance after failed swap");
    assert_eq!(to_balance, FPDecimal::zero(), "wrong to balance after failed swap");

    let contract_balances_after = query_all_bank_balances(&bank, contr_addr.as_str());
    assert_eq!(contract_balances_after.len(), 1, "wrong number of denoms in contract balances");
    assert_eq!(contract_balances_after, contract_balances_before, "contract balance has changed after failed swap");
}
...
fn min_quantity_cannot_be_reached() {
    ...
    assert_eq!(execute_result.unwrap_err(), RunnerError::ExecuteError { msg: format!("failed to execute message; message index: 0: dispatch: submessages: reply: dispatch: submessages: reply: Min expected swap amount ({min_quantity}) not reached: execute wasm contract failed").to_string() }, "wrong error message");
...
fn paused_market() {
    ...
    let signer = must_init_account_with_funds(&app,&[coin(1_000_000_000_000_000_000_000_000, INJ)]);
    ...
    assert!(query_result.unwrap_err().to_string().contains("Querier contract error"), "wrong error returned by query");
    ...
    assert!(execute_result.unwrap_err().to_string().contains("Querier contract error"), "wrong error returned by execute");
...
#[test]
fn swap_same_denom_to_same_denom() {
    let app = InjectiveTestApp::new();
    let wasm = Wasm::new(&app);
    let exchange = Exchange::new(&app);
    let bank = Bank::new(&app);
    ...
    let contr_addr = init_contract_and_get_address(&wasm, &owner, &[coin(10_000_000_000, USDT)]);
    set_route_and_assert_success(&wasm, &owner, &contr_addr, ETH, ETH, vec![spot_market_1_id.as_str().into()]);

    let swapper = must_init_account_with_funds(&app,&[
        coin(12, ETH),
        coin(100_000_000_000_000, INJ), //to pay for gas
    ]);
    ...
    create_limit_order(&app, &trader1, &spot_market_1_id, OrderSide::Sell, 210000, 3);
    create_limit_order(&app, &trader2, &spot_market_1_id, OrderSide::Sell, 205000, 5);
    create_limit_order(&app, &trader3, &spot_market_1_id, OrderSide::Sell, 20200, 2);
    ...
    let execute_result = wasm.execute(
        ...
        &ExecuteMsg::Swap {
            target_denom: ETH.to_string(),
            min_quantity: FPDecimal::from(1u128),
        },
        &vec![coin(8, ETH)],
        &swapper,
    );
